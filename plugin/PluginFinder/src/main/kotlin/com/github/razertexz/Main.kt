...
class Main : Plugin() {
    override fun start(context: Context) {
        ...
        patcher.patch(WidgetSettings::class.java.getDeclaredMethod("onViewBound", View::class.java), Hook {
            ...
            val layoutContext = layout.context

            TextView(layoutContext, null, 0, R.i.UiKit_Settings_Item_Icon).run {
                text = "Open Plugin Finder"
                setTypeface(font)

                icon.setTint(ColorCompat.getThemedColor(layoutContext, R.b.colorInteractiveNormal))
